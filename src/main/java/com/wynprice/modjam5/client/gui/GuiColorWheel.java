...
public class GuiColorWheel extends GuiScreen
{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) 
	{
		...
		Minecraft.getMinecraft().getRenderItem().renderItemAndEffectIntoGUI(ColorUtils.setColor(new ItemStack(WorldPaintItems.COLORPICKER), prevColor), this.width / 80 * 3 - 8, this.height / 10 - 8);		
		GlStateManager.scale(-5, -5, -5);
	...
	public void onGuiClosed() {
		WorldPaintNetwork.sendToServer(new MessagePacketColorGuiClosed(hand, prevColor, new Point(currentColorWheel.x - Minecraft.getMinecraft().displayWidth / 2, currentColorWheel.y - Minecraft.getMinecraft().displayHeight / 2), barAmount));
		super.onGuiClosed();
	}
